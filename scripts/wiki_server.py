...
try:
    ...
    import wiki_rerank as _wiki_rerank
...
async def _get_embed_model():
    ...
    async with _embed_model_lock:
        if _embed_model is None:
            ...
            device = _cfg.get("device")  # None → sentence-transformers auto-sceglie CUDA se disponibile
...
async def api_context(request: Request, q: str = "", k: int = 3, max_chars: int = 600):
    ...
    try:
        ...
        client = _qdrant_get_db(_cfg)
        raw = _qdrant_query_similar(client, _cfg, vector, k=k * 8)

        exclude_patterns = _cfg.get("exclude_from_index", [])
        candidates = [
            r for r in raw
            if r.get("chunk_text") and not any(_fnmatch.fnmatch(r["path"], p) for p in exclude_patterns)
        ]
        candidates = await asyncio.get_event_loop().run_in_executor(
            None, lambda: _wiki_rerank.rerank(q, candidates, _cfg)
        )
        ...
        for r in candidates:
            ...
            if path not in seen or r["_rerank_score"] > seen[path]["score"]:
                seen[path] = {"score": r["_rerank_score"], "chunk_text": r["chunk_text"][:max_chars]}

        top = sorted(seen.items(), key=lambda x: x[1]["score"], reverse=True)[:k]
