...
def cmd_query(args, cfg):
    ...
    results = query_similar(db, cfg, vector, k=args.k * 8)
    results = wiki_rerank.rerank(args.q, results, cfg)[: args.k]
    ...
    ok({"op": "query", "results": [
        {"path": r["path"], "chunk_id": r["chunk_id"],
         "score": float(r.get("_rerank_score", 0)), "excerpt": r["chunk_text"][:200]}
        for r in results
    ]})
